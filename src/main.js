    ...
    case "KAN":
        {
            ...
            // 直接用 actionCheck 計算好的候選（避免 null / 拿錯資料）
            const list = ui.state.kanTiles || [];
            const tileToKan = list.length ? list[0] : null;

            if (tileToKan == null) {
                console.warn("KAN pressed but kanTiles is empty.");
                return;
            }

            game.playerAction("KAN", {
                kanTile: tileToKan
            });
            return;
        }
    ...
    case "TSUMO":
        {
            ...
            if (ui.state.type !== "DRAW") return;
            game.playerAction("TSUMO", {
                winTile: ui.state.incomingTile
            });
            return;
        }
